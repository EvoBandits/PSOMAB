...
int main() {
    ...
    std::vector<int> x_ub{100,100,100,100,100, 100, 100, 100, 100, 100, 100,100,100,100,100,100,100,100,100,100};*/
        ...
        for (unsigned i = 1; i <= number_of_samples; i++) {
                ...
                PSOMAB instance = PSOMAB(inventory, 10000, m, 1, seed, x_lb, x_ub, 2);//itarations, population_size, mutation_rate, crossover_rate, mutation_span , objective ( 0=min, 1=max), 1== sim obs
                ...
                for (int k = 0; k < instance.best_solutions.size(); k++) {
                        ...
                        std::cout << i << "/" << number_of_samples << " runs|"
                                  << "budget:" << instance.best_solutions.at(k).obs_number << " | true:" << instance.best_solutions.at(k).true_func_val << "  mean:" << instance.best_solutions.at(k).mean_func_val << "  ";
