...
double get_true_objective_value_eggholder(const Eigen::VectorXi &action_vector) {
        ...
        const double result = -(action_vector_scaled[1] + 47) * sin(sqrt(abs(action_vector_scaled[0] / 2 + (action_vector_scaled[1] + 47)))) - action_vector_scaled[0] * sin(sqrt(abs(action_vector_scaled[0] - (action_vector_scaled[1] + 47))));
        return result;
}
