...
int128_t PSOMAB::calc_solution_code(std::vector<int> x) {
  ...
  for (int i = 0; i < dim; i++) {
    ...
    int base = 10;
    search_index =
        search_index + (int128_t) (pow((pow(10, exp)), (dim - 1) - i) * (x[i] - vec_x_min[i]));
  }
  return search_index;
}
      ...
      if (arm_index >= 0) {// alle arm_index >= 0 existieren, -1 falls arm noch
        ...
        if (!check_if_arm_was_in_current_arms) {
          ...
          std::multiset<MS_element, std::less<>>::iterator it = MS_vec[i].find(
              MS_element(arm_index, arms_vec[i].at(arm_index).get_r() / arms_vec[i].at(arm_index).get_k()));
          ...
          std::multiset<MS_element, std::less<>>::iterator it_global =
              MS_global.find(
                  MS_element(arm_index_global,
                             arms_global.at(arm_index_global).get_r() / arms_global.at(arm_index_global).get_k()));
          ...
          arms_global[arm_index_global].update_r(
              r_after_update - r_before_update);// Im Globalen Arm Gedächtnis muss der Sample
          ...
          MS_global.insert(
              MS_element(var_global,
                         arms_global.at(var_global).get_r() / arms_global.at(var_global).get_k()));// füge neu zu MS_global hinzu
          //////end global////////
        ...
        if (arm_index_global >= 0) {// falls knoten bereits im GLOBAL LUT existiert
          ...
          MS_global.insert(
              MS_element(var_global, arms_global.at(var_global).get_r() / arms_global.at(var_global).get_k()));
          ...
          MS_global.insert(MS_element(
              new_index_global, arms_global.at(new_index_global).get_r() / arms_global.at(new_index_global).get_k()));
        ...
        MS_vec[i].insert(
            MS_element(new_index, arms_vec[i].at(new_index).get_r() / arms_vec[i].at(new_index).get_k()));
      }
    }
    //////////////////////////////////////////////////////////////////////////////////////
    ...
    for (unsigned int i = 0; i < current_particles.size(); i++) {
      std::multiset<MS_element, std::less<>>::iterator it =
          MS_vec[i].find(MS_element(
              ...
              arms_vec[i].at(best_individual_arm_indices[i]).get_r() / arms_vec[i].at(best_individual_arm_indices[i]).get_k()));
      int var = (*it).arm_index;
      ...
      std::multiset<MS_element, std::less<>>::iterator it_global =
          MS_global.find(MS_element(
              arm_index_global, arms_global.at(arm_index_global).get_r() / arms_global.at(arm_index_global).get_k()));
      int var_global = (*it_global).arm_index;
...
void PSOMAB::print_Q_tree_global() {
  ...
  for (int o = 0; o < MS_global.size(); o++) {
    ...
    std::cout << arms_global.at(arm_index).get_r() / arms_global.at(arm_index).get_k()
              << std::endl;
...
void PSOMAB::save_solution(int z) {// z iterationszahl
  ...
  while (last_element_reached == false) {
    ...
    if (1 - (ucb_norm_max - arms_global.at(arm_index).get_r() / arms_global.at(arm_index).get_k()) / (ucb_norm_max - ucb_norm_min) + sqrt(2 * log(sim_counter) / arms_global.at(arm_index).get_k()) < best_ucb_value) {
      best_ucb_value =
          1 - (ucb_norm_max - arms_global.at(arm_index).get_r() / arms_global.at(arm_index).get_k()) / (ucb_norm_max - ucb_norm_min) + sqrt(2 * log(sim_counter) / arms_global.at(arm_index).get_k());
  ...
  if (stopping_criterion == 1) {// falls nach max anzahl an simulation observations abgebrochen wird
    ...
    best_solutions.push_back(
        solution(sim_counter, arms_global.at(return_index2).get_action_vector(),
                 ...
                 arms_global.at(return_index2).get_r() / arms_global.at(return_index2).get_k(),
                 true_value));
