...
double normal_random_number_for_mutation(double m, double s){
    std::normal_distribution<double> random_normal(m,s);
    return random_normal(e);
};
...
int PSOMAB::run()
{
    int save_solution_every_x=100; // if stopping criterion = max number of sim. obs.: each 100th solution is stored for "log-file" data
    ...
    for(int z=1; z<=max_iter_or_sim_number; z++){
        if(z==1){  // first iteration   !!!LOGIK IN DER ERSTEN ITERATION!!!
            for(int i=0; i<arms.size(); i++){
                ...
                lookuptree_vec.push_back(lookuptree_temp);  //Füge (lokalen) LUT dem Vektor aller lokalen LUTs hinzu
                // insert into Lookuptree (LUT)
                int128_t search_index=calc_solution_code(arms.at(i).get_action_vector()); //Berechne "unique integer" bzw. search key/index
                lookuptree_vec.at(i).insert(0, search_index); // Füge einen neuen Knoten mit (search index, 0) dem lokalen LUT hinzu.  "0" deshalb, da es der erste Knoten ist.
                ...
                arms_vec.push_back(arms_temp); // arms_vec[i]: Arm-Gedächtnis (Arm-Vektor) des i-ten Partikels
                arms_vec[i].push_back(arms.at(i)); // Füge Arm "arms.at(i)" dem Gedächtnis des i-ten Partikels hinzu
                
                arms_vec[i].at(0).pull_arm(obj); // Ziehe entsprechenden arm               0: Es gibt arms.size() speicher_listen, in jeder liste wird hier nur das 1. Element befüllt.
                sim_counter+=1; // erhöhe den totalen sim obs. counter
                //////////////
                if(stopping_criterion==1){ //stop after max observations
                    if(sim_counter%save_solution_every_x==0){ save_solution(z);}  // save solution
                    if(sim_counter==max_iter_or_sim_number){  return 0;} 
                }
                ...
                MS_vec[i].insert(MS_element(0, Q_PSO));  // // Füge einen neuen Knoten mit (Q:PSO:sample mean, 0) dem lokalen SAT hinzu.     0 : Index des ersten Arms (in jeder der arms.size() Listen = diese sind partikelspezifisch)
                
                
                ///global
                int128_t search_index_global=calc_solution_code(arms_vec[i].at(0).get_action_vector());  //berechne "unique integer" aka search index
                lookuptree_global.insert(i, search_index);  // füge entsprechenden knoten in den GLOBALEN LUT
                arms_global.push_back(arms_vec[i].at(0));  // füge den arm (zugehörig zum Knoten) in das GLOBALE Arm gedächntis
                MS_global.insert(MS_element(i, Q_PSO));    // füge einen entsprechenden Knoten in den GLOBALEN SAT ein (i: Index im globalen Arm Gedächtnis, Q_PSO: Sample Mean)   //  hier i oben 0, da hier ->globaler <- Baum aufgebaut wird          
            }
        }
        ...
        if(z==1){
                save_solution(z); // save the solution that is currently considered to be the best
        }
        ...
        best_global_Q=1000000000;  //hier muss NICHT zwischen min oder max optimierungsproblem unterschieden werden. 
        //                         in Arm Funktion werden max probleme mit (-1) multiplikation zu äquiv. min problemen gemacht, deshalb ist hier ein hoher positiver Q Wert immer richtig. 
        int best_global_index; // index des glabal best arm
        int best_global_particle_index; // Welches der insgesamt m Partel den global best arm enthält
        
        for(int k=0; k<m; k++){
            
            std::multiset<MS_element, std::less<>>::iterator it_PSO= MS_vec[k].begin();
        ...
        //////////////////////////////////////////////////////////////////////////////////////
        /////////////////////////AB HIER HIER EIGENTLICHE PSO FUNKTIONALITÄT/////////////////
        //////////////////////////////////////////////////////////////////////////////////////
        for(int k=0; k<m; k++){
            ...
            for(int g=0; g<current_particles.at(k).get_action_vector().size(); g++){
                int b1=round(random_number_unif(0,c1)*1.0*(best_individual_arms[k].get_action_vector()[g]-current_particles[k].get_action_vector()[g]));
                int b2=round(random_number_unif(0,c2)*1.0*(arms_vec[best_global_particle_index][best_global_index].get_action_vector()[g]-current_particles[k].get_action_vector()[g]));
            ...
            for(int g=0; g<current_particles.at(k).get_action_vector().size(); g++){
                new_velocity.push_back(round(v*velocity[k][g]+best_individual_diff[g]+best_global_diff[g]));
        ...
        //////////////////////////////////////////////////////////////////////////////////////
        /////////////////////////BIS HIER EIGENTLICHE PSO FUNKTIONALITÄT//////////////////////
        //////////////////////////////////////////////////////////////////////////////////////
        ...
        //////////////////////////////////////////////////////////////////////////////////////
        /////////////////////////AB HIER HIER PRIMÄR MEMORY (d.h. MAB) FUNKTIONALITÄT//////////
        //////////////////////////////////////////////////////////////////////////////////////
        for(unsigned int i=0; i<current_particles.size(); i++){
            ...
            if(arm_index>=0){  // alle arm_index >= 0 existieren, -1 falls arm noch nicht vorhanden
                // existiert bereits
                        ...
                        if(!check_if_arm_was_in_current_arms){
                            ...
                            std::multiset<MS_element, std::less<>>::iterator it=MS_vec[i].find(MS_element(arm_index, arms_vec[i].at(arm_index).get_r()/ arms_vec[i].at(arm_index).get_k())); 
                            //Suche im lokalen SAT des i-ten Partikel nach entsprechendem Knoten
                            int var = (*it).arm_index;
                            while(var!=arm_index){ // If two nodes have the same mean value, the case var!=arm_index max occur
                            ...
                            const int arm_index_global=lookuptree_global.search(search_index); // // Suche entsprechenden Arm im global LUT
                                    // arm_index_global=suche von arms_vec[i].at(var) den index im lookuptree_global
                            std::multiset<MS_element, std::less<>>::iterator it_global=MS_global.find(MS_element(arm_index_global, arms_global.at(arm_index_global).get_r()/ arms_global.at(arm_index_global).get_k()));
                            //Suche im GLOBALEN SAT nach entsprechendem Knoten
                            int var_global = (*it_global).arm_index;
                            while(var_global!=arm_index_global){ // If two nodes have the same mean value, the case var!=arm_index max occur
                                //In this case, the tree must be iterated further until var==arm_index in order to actually find the correct (SAT) node
                                it_global++;
                            ...
                            arms_global[arm_index_global].update_k(1); // erhöhe um 1 // IM Globalen Arm Gedächtnis muss der sim counter um 1 erhöht werden, da der arm (wenige zeilen weiter oben) gezogen worden ist. 
                            // erhöhe bei arms_global[arm_index_global] k um eins
                            
                            arms_global[arm_index_global].update_r(r_after_update-r_before_update); // Im Globalen Arm Gedächtnis muss der Sample mean geupdatet werden, da (wenige zeilen weiter oben) der Arm gezogen worden ist.
                            ...
                            if(stopping_criterion==1){ //stop after max observations
                                if(sim_counter%save_solution_every_x==0){ save_solution(z);}  // save solution
                                if(sim_counter==max_iter_or_sim_number){  return 0;} // max_generations ist im fall von stopping_criterion==1 ein dummy für die max anzahl an observations
                            }
                            //////////////
                            
                            MS_vec[i].insert(MS_element(var, arms_vec[i].at(var).get_r()/ arms_vec[i].at(var).get_k())); // Füge gezogenen Arm dem Lokal SAT hinzu (er wurde zuvor aus dem lok. SAT entfernt).
                            
                        }
            }
            else{
                // existiert noch nicht
                Arm new_arm(current_particles[i].get_action_vector(), 0);  // 0 = cost info, eigentlich nicht notwendig


                                
                arms_vec[i].push_back(new_arm);  //füge Arm dem lokalen Arm Gedächtnis des i-ten Partikel zu
                ...
                int128_t search_index_global=calc_solution_code(arms_vec[i].back().get_action_vector()); //  berechne "unique integer" aka search index
                const int arm_index_global=lookuptree_global.search(search_index_global);  // arm_index_global=suche von arms_vec[i].at(var) den index im lookuptree_GLOBAL
                if(arm_index_global>=0){ //falls knoten bereits im GLOBAL LUT existiert
                    std::multiset<MS_element, std::less<>>::iterator it_global=MS_global.find(MS_element(arm_index_global, arms_global.at(arm_index_global).get_r()/ arms_global.at(arm_index_global).get_k()));
                    int var_global = (*it_global).arm_index;
                    while(var_global!=arm_index_global){ // Falls zwei Lösungen den selben Mean Value haben,
                    // kann prinzipiell var!=arm_index auftreten. Dann muss im Baum weiter iteriert werden bis var==arm_index um wirklich die richtige Lösung zu ziehen.
                        it_global++;
                        var_global=(*it_global).arm_index;
                    }
                    MS_global.erase(it_global);        
                    // lösche knoten zugehörig zu arm_index_global in MS_global (er wird gezogen und verändert sich. für das update muss man ihn deshalb löschen)
                ...
                //root = (*root).InsertIntoTree(root, search_index, new_index); // fill search tree ,  (arms.size()-1) ist index des letzten Elements
        ...
        //////////////////////////////////////////////////////////////////////////////////////
        /////////////////////////BIS HIER HIER PRIMÄR MEMORY FUNKTIONALITÄT///////////////////
        //////////////////////////////////////////////////////////////////////////////////////
        ...
        // Die m besten Arme  werden in jeder Iteration erneut gezogen um bessere Sample Means zu erhalten. Das passiert in der folgenden For loop
        for(unsigned int i=0; i<current_particles.size(); i++){
            std::multiset<MS_element, std::less<>>::iterator it=MS_vec[i].find(MS_element(best_individual_arm_indices[i], arms_vec[i].at(best_individual_arm_indices[i]).get_r()/ arms_vec[i].at(best_individual_arm_indices[i]).get_k()));
            int var = (*it).arm_index;
            while(var!=best_individual_arm_indices[i]){ // Falls zwei Lösungen den selben Mean Value haben,
            // kann prinzipiell var!=arm_index auftreten. Dann muss im Baum weiter iteriert werden bis var==arm_index um wirklich die richtige Lösung zu ziehen.
                it++;
                var=(*it).arm_index;
            }
...
std::vector<std::vector<int>> PSOMAB::get_history(){ // gibt die gesamte Historie aller Arme zurück, in der Form: {Anzahl besuche| Aktionen}
...
// prints all nodes of global MS. The first node is the node/solution considered to be best.
void PSOMAB::print_Q_tree_global(){
            ...
            for(int o=0; o<MS_global.size(); o++){
                ...
                std::cout << arms_global.at(arm_index).get_r()/arms_global.at(arm_index).get_k() << std::endl;             
...
void PSOMAB::print_Q_tree(int i){
            ...
            for(int o=0; o<MS_vec[i].size(); o++){
                ...
                std::cout <<" Q: ";
                std::cout << arms_vec[i].at(arm_index).get_r()/arms_vec[i].at(arm_index).get_k();
...
void PSOMAB::save_diversity(std::vector<int> indices, int total_rep){
    ...
    for(int i=0; i<indices.size(); i++){
        for(int j=0; j<arms[indices[i]].get_action_vector().size(); j++){
            variances[j]+= pow(means[j]-arms[indices[i]].get_action_vector()[j],2);
        }
    } 
    int n= indices.size();
...
void PSOMAB::save_timesteps(int z){
    for(int v=1; v<101;v++){
        if(arms.size()>=v*1000 and V<v*1000){
            ...
            timestamps.push_back({duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count(),size,z});
        }
    }
    V=arms.size();
}
...
void PSOMAB::save_solution(int z){ //z iterationszahl
            ...
            for(int o=0; o<MS_global.size(); o++){
                ...
                //For UCB-normalized approach only
                if(arms_global.at(arm_index).get_k() > max_number_pulls){ max_number_pulls=arms_global.at(arm_index).get_k(); }
                //For UCB-normalized approach only                
            ...
            while(last_element_reached==false){
                ...
                if(arms_global.at(arm_index).get_r()/arms_global.at(arm_index).get_k() < ucb_norm_min){   ucb_norm_min=arms_global.at(arm_index).get_r()/arms_global.at(arm_index).get_k();         }
                if(arms_global.at(arm_index).get_r()/arms_global.at(arm_index).get_k() > ucb_norm_max){   ucb_norm_max=arms_global.at(arm_index).get_r()/arms_global.at(arm_index).get_k();         }
            ...
            if(obj==0){ // minimization Problem
                ...
                while(last_element_reached==false){
                    ...
                    if(ucb_norm_max==ucb_norm_min){return_index2=arm_index;} //sonst wäre Nenner W_k - B_k null und in oberer if bedingung würde inf < minvalue2 stehen
                    it_norm2++;
                }  
            }
                ...
                while(last_element_reached==false){
                    ...
                    if(   (ucb_norm_max-arms_global.at(arm_index).get_r()/arms_global.at(arm_index).get_k())/(ucb_norm_max-ucb_norm_min) - sqrt(2*log(sim_counter)/arms_global.at(arm_index).get_k()) > best_ucb_value){
                        best_ucb_value=(ucb_norm_max-arms_global.at(arm_index).get_r()/arms_global.at(arm_index).get_k())/(ucb_norm_max-ucb_norm_min) - sqrt(2*log(sim_counter)/arms_global.at(arm_index).get_k());
                        return_index2=arm_index;
                    }
                    if(ucb_norm_max==ucb_norm_min){return_index2=arm_index;} //sonst wäre Nenner W_k - B_k null und in oberer if bedingung würde inf < minvalue2 stehen
                    it_norm2++;
            ...
            double true_value=arms_global.at(return_index2).function_value(obj,false); //no noise
            /// TRUE VALUE EINFACH AUF EINEN BELIEBIGEN WERT SETZEN; FALLS SIMULATION ZU RECHENINTENSIV IST UND EXAKTER WERT OHNEHIN NICHT BEKANNT/BESTIMMBAR
            

            if(stopping_criterion==1){ //falls nach max anzahl an simulation observations abgebrochen wird (wird sim_counter an erster stelle angezeigt)
                best_solutions.push_back(  solution( sim_counter, arms_global.at(return_index2).get_action_vector(), arms_global.at(return_index2).get_k(), arms_global.at(return_index2).get_r()/arms_global.at(return_index2).get_k(),true_value ));
            }
            else{  //d.h. falls nach max anzahl an iterationen abgebrochen wird (wird z = iterationszahl an erster stelle angezeigt)
                best_solutions.push_back(  solution( z, arms_global.at(return_index2).get_action_vector(), arms_global.at(return_index2).get_k(), arms_global.at(return_index2).get_r()/arms_global.at(return_index2).get_k(), true_value ));
            }
            

            //std::cout <<  best_solutions.back().obs_number << " " << arms.at(return_index2).get_action_vector().at(0)<< ","<< arms.at(return_index2).get_action_vector().at(1) << "   N:" << arms.at(return_index2).get_k()<< "  Q:"<< arms.at(return_index2).get_r()/arms.at(return_index2).get_k() << "  True:"<< true_value<<std::endl;
}
...
PSOMAB::PSOMAB(unsigned long max_gen, int pop_s, double mutation, double crossover, double mu_span, int objective, int stopping_criterion,  unsigned s, std::vector<int> x_lb, std::vector<int> x_ub): max_iter_or_sim_number{max_gen} , m{pop_s}, mutation_rate(mutation), crossover_rate(crossover), mutation_span(mu_span), obj(objective), stopping_criterion(stopping_criterion)
{
    e.seed(s); //initialize seed
    ...
    for(int i=0; i<pop_s; i++){
        ...
        while(stop_while==false){
            bool is_unique=true;
            for(int j=0;j<init_solutions.size(); j++){  // Passes through all previous solutions within init_solutions
                bool all_same=true;
                for(int k=0; k<x_lb.size(); k++){
                    if(v[k]!=init_solutions[j][k]){
                        all_same=false;
                    }
                }
                if(all_same){
        ...
        Arm new_arm(v, 0);  // 0 = cost info, last element (0) is actually not necessary
